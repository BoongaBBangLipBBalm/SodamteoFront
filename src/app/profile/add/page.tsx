...
const ProfileEdit: React.FC = () => {
  ...
  const [tempMinValue, tempSetMinValue] = useState(-1);
  const [tempMaxValue, tempSetMaxValue] = useState(-1);
  const [tempIsEnabled, tempSetIsEnabled] = useState(true);

  const [humidMinValue, humidSetMinValue] = useState(-1);
  const [humidMaxValue, humidSetMaxValue] = useState(-1);
  const [humidIsEnabled, humidSetIsEnabled] = useState(true);

  const [sunLightMinValue, sunLightSetMinValue] = useState(-1);
  const [sunLightMaxValue, sunLightSetMaxValue] = useState(-1);
